...
class StandaloneMorkBot:
    ...
    def get_updates(self):
        ...
        try:
            ...
            response = requests.get(url, params={"timeout": 25, "offset": self.offset}, timeout=30)

            if response.status_code == 409:
                logger.error(
                    "[poll] 409 Conflict from Telegram: webhook set or another poller is consuming updates."
                )
                logger.error(
                    '[poll] run: curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/deleteWebhook"'
                )
                self.running = False
                return {"ok": False}
