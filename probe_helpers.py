...
def probe_pumpfun_sources(limit: int = 50) -> dict:
        ...
        try:
            with httpx.Client(timeout=8.0) as c:
                ...
                if r.status_code == 200:
                    ...
                    for it in items[:3]:
                        samples.append({k: it.get(k) for k in ("symbol","name","mint","market_cap","liquidity","created_timestamp") if k in it})
                else:
                    err = f"HTTP {r.status_code}"
        except Exception as e:
            err = str(e)
        dt = round((time.time() - t0) * 1000)
        results["sources"].append({"label": label, "url": url, "ms": dt, "status": code, "count": n, "error": err, "samples": samples})

    # 1) Worker (if set)
    if PUMPFUN_FALLBACK:
        _try("pumpfun-via-worker", PUMPFUN_FALLBACK.rstrip("/") + "/pf/coins/created", {"limit": limit, "offset": 0})
    ...
    rpc = {"label": "solana-rpc", "url": HEL_RPC or "(unset)", "ok": False, "ms": None, "error": None}
    if HEL_RPC:
        t0 = time.time()
        try:
