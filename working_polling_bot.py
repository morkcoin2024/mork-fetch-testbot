...
def _notify_admin(msg: str):
    try:
        ...
        requests.post(
            url, json={"chat_id": admin, "text": f"⚠️ F.E.T.C.H alert:\n{msg}"}, timeout=10
        )
    except Exception:
        pass
        ...
        try:
            ...
            try:
                ...
                if result:
                    ...
                    self.send_telegram_message(
                        chat_id,
                        f"Command '{text}' not recognized. Use /help for available commands.",
                    )

            except Exception as e:
                logger.error(f"Command processing error: {e}")
                self.send_telegram_message(
                    chat_id, "⚠️ Command processing error. Bot is still operational."
                )
        ...
        try:
            ...
            if not response.ok:
                if response.status_code == 409:
                    ...
                    _notify_admin(
                        "409 Conflict from Telegram (webhook set or another poller). Poller stopping."
                    )
                    self.running = False
                    return
