...
def _get_json(url: str, params: dict | None = None, timeout: int = 10) -> dict | None:
    """Legacy helper for backward compatibility."""
    return _get_json_retry(url, params=params, timeout=timeout)


def _minutes_since_ms(timestamp_ms: int | None) -> int | None:
    ...
    for base in PUMPFUN_ENDPOINTS:
        try:
            ...
            if items:
                ...
                publish(
                    "fetch.pumpfun.yield",
                    {"tokens": len(items), "endpoint": base, "limit": limit, "offset": offset},
                )
                return items
        except Exception as e:
            ...
            # Publish fetch failure event
            publish(
                "fetch.pumpfun.status", {"status": "fail", "code": "exception", "error": str(e)}
            )
...
def fetch_candidates_from_dexscreener(
    limit: int = 50, max_pairs: int = 500
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        # Publish fetch failure event
        publish(
            "fetch.dexscreener.status", {"status": "fail", "code": "exception", "error": str(e)}
        )
        return []
...
def _dedupe_keep_best(tokens: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for token in tokens:
        ...
        if mint not in seen_mints:
            ...
            elif token.get("risk", 100) < existing.get("risk", 100):
                seen_mints[mint] = token
...
def multi_source_fetch(limit=10, force=False):
    ...
    try:
        ...
        logging.info(
            f"[MULTI-FETCH] Complete: {len(final_tokens)} tokens from {len(sources_used)} sources"
        )
        return result

    except Exception as e:
        logging.error(f"[MULTI-FETCH] Error: {e}")
        return {"total": 0, "sources": [], "tokens": [], "status": "error", "error": str(e)}
...
def fetch_and_rank(rules):
    ...
    try:
        ...
        if enriched_pumpfun:
            ...
            publish(
                "source_complete",
                {"source": "pumpfun-enriched", "count": len(enriched_pumpfun), "status": "success"},
            )
        else:
            publish(
                "source_complete", {"source": "pumpfun-enriched", "count": 0, "status": "empty"}
            )
    except Exception as e:
        logging.warning("Enriched Pump.fun fetch failed, falling back to standard: %s", e)
        publish(
            "source_complete",
            {"source": "pumpfun-enriched", "count": 0, "status": "failed", "error": str(e)},
        )
    ...
    try:
        ...
        publish(
            "source_complete",
            {"source": "on-chain", "count": len(chain_items), "status": "success"},
        )
    except Exception as e:
    ...
    try:
        ...
        publish(
            "source_complete",
            {"source": "pumpfun", "count": len(pumpfun_rows), "status": "success"},
        )
    except Exception as e:
    ...
    if not pumpfun_rows:
        ...
        try:
            ...
            publish(
                "fallback_activated",
                {"source": "on-chain", "count": len(seeds), "reason": "pumpfun_api_failed"},
            )
        except Exception as e:
    ...
    try:
        ...
        publish(
            "source_complete",
            {"source": "dexscreener", "count": len(dex_items), "status": "success"},
        )
    except Exception as e:
    ...
    filtered.sort(
        key=lambda x: (_src_priority(x.get("source")), x["risk"], -(x.get("liquidity_usd") or 0))
    )
    ...
    publish(
        "fetch_completed",
        {
            ...
            "sources": sources,
            "top_tokens": [
                {"symbol": t.get("symbol"), "source": t.get("source"), "risk": t.get("risk")}
                for t in filtered[:5]
            ],
        },
    )
