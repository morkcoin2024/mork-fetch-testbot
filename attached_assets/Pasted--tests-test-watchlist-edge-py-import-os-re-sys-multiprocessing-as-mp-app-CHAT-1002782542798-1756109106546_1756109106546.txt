...
def data_rows(s):  # lines that look like watchlist rows
...
# 2) Empty watchlist should still render headers for all modes, with no data rows
...
# 3) Sorting toggles render correctly when empty
hdr = send("/watchlist volumes desc").splitlines()[0]
fails += ck("(desc)" in hdr, "sort volumes desc (empty)")
hdr = send("/watchlist supply asc").splitlines()[0]
fails += ck("(asc)" in hdr, "sort supply asc (empty)")

# 4) Add a token twice; watchlist should at least contain it (no crash/dup loop)
send(f"/watch {SOL_L}")
send(f"/watch {SOL_L}")  # add twice on purpose
