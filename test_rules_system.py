...
def test_rules_system():
    ...
    if conservative_results["tokens"]:
        ...
        print(
            f"   🏆 Top token: {best_token['symbol']} - Score: {best_token['score_total']:.1f}/100"
        )
    print()
    ...
    for category, data in analysis["score_breakdown"].items():
        print(
            f"      • {category.title()}: {data['score']:.1f}/100 (weight: {data['weight']:.1f}%)"
        )
    print()
    ...
    print(
        f"✅ Conservative profile found {conservative_results['returned_count']} qualifying tokens"
    )
