...
def enrich_with_solscan(tokens: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        log.info("[ENRICH] Solscan enrichment complete: %d/%d tokens enriched", 
                enriched_count, len(tokens))
        return tokens
...
def test_enrichment(limit: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        results = {
            ...
            "enrichment_rate": f"{(enriched_count/len(sample_tokens)*100):.1f}%" if sample_tokens else "0%",
            "sample_enriched": []
        }
