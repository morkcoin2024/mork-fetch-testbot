...
class SafetySystem:
    ...
    def check_mork_holdings(self, wallet_address: str, required_sol_worth: float) -> Tuple[bool, str]:
        ...
        try:
            ...
            if is_routable and "tokens expected" in route_msg:
                try:
                    ...
                    tokens_per_milli_sol = int(route_msg.split("tokens expected")[0].split(" - ")[-1].replace(",", ""))
                    ...
                    if mork_balance >= required_mork_tokens:
                        ...
                        return True, f"✅ MORK verified: {sol_worth:.3f} SOL worth (need {required_sol_worth})"
                    else:
                        sol_worth = mork_balance / tokens_per_sol
                        return False, f"❌ Insufficient MORK: {sol_worth:.3f} SOL worth (need {required_sol_worth})"
                except:
    ...
    def check_daily_limits(self, chat_id: str, amount_sol: float) -> Tuple[bool, str]:
        ...
        try:
            ...
            if current_spending + amount_sol > self.daily_spend_limit:
                return False, f"Daily limit exceeded: {current_spending:.3f} + {amount_sol:.3f} > {self.daily_spend_limit}"
    ...
    def comprehensive_safety_check(self, chat_id: str, wallet_address: str, 
                                  token_mint: str, amount_sol: float, 
                                  feature_type: str = "snipe") -> Tuple[bool, str]:
        ...
        # MORK holdings gate
        required_mork = self.min_mork_for_fetch if feature_type == "fetch" else self.min_mork_for_snipe
