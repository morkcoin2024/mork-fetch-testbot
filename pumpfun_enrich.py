        ...
        for attempt in range(retries):
            try:
                ...
                if r.status_code == 200:
                    ...
                    publish(
                        "pumpfun.raw", {"n": len(data), "endpoint": endpoint_name, "url": base_url}
                    )
                    return data
                else:
                    publish(
                        "pumpfun.err",
                        {"code": r.status_code, "endpoint": endpoint_name, "attempt": attempt + 1},
                    )
                ...
                publish(
                    "pumpfun.exc",
                    {"err": str(e), "endpoint": endpoint_name, "attempt": attempt + 1},
                )
            ...
            for attempt in range(retries):
                try:
                    ...
                    if r.status_code == 200:
                        js = r.json()
                        items = (
                            js if isinstance(js, list) else js.get("coins") or js.get("data") or []
                        )
        ...
        with httpx.Client(timeout=timeout, headers={"content-type": "application/json"}) as c:
            for i in range(0, len(mints), batch_size):
                ...
                for idx, mint in enumerate(chunk):
                    calls.append(
                        {"jsonrpc": "2.0", "id": idx, "method": "getTokenSupply", "params": [mint]}
                    )
...
def search_dexscreener(query, limit=100):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            publish(
                "dex.search.error",
                {"query": query, "code": r.status_code, "response_time": response_time},
            )
...
def bulk_enrich_pipeline(limit=200, include_search=True):
    ...
    publish("bulk.pipeline.started", {"limit": limit, "include_search": include_search})
    ...
    publish(
        "bulk.pipeline.completed",
        {
            ...
            "tokens_per_second": (
                round(results["total_unique_tokens"] / processing_time, 2)
    ...
    logging.info(
        f"Bulk enrichment complete: {results['total_unique_tokens']} unique tokens in {processing_time}s"
    )
    return results
