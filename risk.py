...
def gas_headroom_ok(pubkey: str, required_sol: float) -> tuple[bool, str]:
    ...
    try:
        ...
        if current_balance >= total_required:
            ...
            return (
                False,
                f"Insufficient SOL: {current_balance:.6f} < {total_required:.6f} (includes {MIN_SOL_HEADROOM} headroom)",
            )
...
def comprehensive_safety_check(
    pubkey: str, mint: str, sol_amount: float, user_spend_cap: float = None
) -> tuple[bool, str]:
    """
