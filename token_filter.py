...
class TokenFilter:
    ...
    def filter_and_score_tokens(self, tokens: List[Dict[str, Any]], 
        ...
        logger.info(f"Hard filters: {results['passed_filters']} passed, {results['failed_filters']} failed")
    ...
    def get_token_analysis(self, token_data: Dict[str, Any], 
                          profile_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        analysis = {
            ...
            "recommendations": self._generate_recommendations(token_data, score_data, filter_reasons)
        }
        
        return analysis
    
    def _generate_recommendations(self, token_data: Dict[str, Any], 
                                score_data: Dict[str, Any], 
                                filter_failures: List[str]) -> List[str]:
        ...
        if low_categories:
            ...
            for category in low_categories[:3]:
                recommendations.append(f"• {category.title()}: {breakdown[category]['score']:.1f}/100")
    ...
    def get_profile_summary(self, profile_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if tokens:
            ...
            for i, token in enumerate(tokens[:5], 1):
                ...
                summary_lines.append(
                    f"{i}. **{symbol}** - Score: {score:.1f}/100"
                )
                summary_lines.append(
                    f"   Liquidity: ${liquidity:,.0f} | Holders: {holders:,}"
                )
...
def quick_filter(tokens: List[Dict[str, Any]], 
                profile: str = "conservative") -> List[Dict[str, Any]]:
