...
class SolanaWalletIntegrator:
    ...
    def _make_rpc_call(self, method: str, params: list) -> dict[str, Any]:
        ...
        for endpoint in SOLANA_RPC_ENDPOINTS:
            try:
                ...
                logging.warning(f"RPC endpoint {endpoint} failed: {e!s}")
                continue
    ...
    def get_sol_balance(self, wallet_address: str) -> float:
        ...
        try:
            ...
            logging.error(f"Error getting SOL balance: {e!s}")
            return 0.0
    ...
    def get_token_balance(self, wallet_address: str, token_mint: str) -> float:
        ...
        try:
            ...
            logging.error(f"Error getting token balance: {e!s}")
            return 0.0
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if pairs:
                        ...
                        sol_pairs = [
                            p for p in pairs if p.get("quoteToken", {}).get("symbol") == "SOL"
                        ]
                        if sol_pairs:
                            best_pair = max(
                                sol_pairs, key=lambda x: float(x.get("liquidity", {}).get("usd", 0))
                            )
                            price = float(best_pair.get("priceNative", 0))
                            if price > 0:
                                logging.info(
                                    f"Found {data.get('pairs', [{}])[0].get('baseToken', {}).get('name', 'Token')}/SOL price: {price} SOL per token"
                                )
                                return price
            except Exception as e:
                logging.warning(f"DexScreener failed: {e!s}")
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success") and "data" in data:
                        ...
                        if usd_price > 0:
                            ...
                            sol_usd_url = (
                                f"https://public-api.birdeye.so/defi/price?address={WSOL_ADDRESS}"
                            )
                            sol_response = self.session.get(
                                sol_usd_url, headers=headers, timeout=10
                            )
                            if sol_response.status_code == 200:
                                ...
                                if sol_data.get("success"):
                                    ...
                                    if sol_usd_price > 0:
                                        ...
                                        logging.info(
                                            f"Found token price via Birdeye: {sol_price} SOL"
                                        )
                                        return sol_price
            except Exception as e:
                logging.warning(f"Birdeye failed: {e!s}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "outAmount" in data:
                        tokens_per_sol = float(data["outAmount"]) / (
                            10 ** int(data.get("outAmountDecimals", 9))
                        )
                        if tokens_per_sol > 0:
                            price_per_token = 1.0 / tokens_per_sol
                            logging.info(
                                f"Found token price via Jupiter Quote: {price_per_token} SOL per token"
                            )
                            return price_per_token
            except Exception as e:
                logging.warning(f"Jupiter Quote API failed: {e!s}")

            logging.error(f"All price sources failed for token {token_mint}")
            return 0.0

        except Exception as e:
            logging.error(f"Critical error getting token price: {e!s}")
            return 0.0
    ...
    def get_recent_transactions(self, wallet_address: str, limit: int = 10) -> list:
        ...
        try:
            result = self._make_rpc_call(
                "getSignaturesForAddress", [wallet_address, {"limit": limit}]
            )
            ...
            for sig_info in signatures[:5]:  # Limit to 5 to avoid rate limits
                try:
                    ...
                    logging.warning(f"Error getting transaction details: {e!s}")
                    continue

            return transactions
        except Exception as e:
            logging.error(f"Error getting recent transactions: {e!s}")
            return []

    def create_swap_transaction_data(
        self,
        wallet_address: str,
        input_mint: str,
        output_mint: str,
        ...
        try:
            ...
            logging.error(f"Error creating swap transaction: {e!s}")
            return None

    def execute_swap_with_signature(
        self, wallet_address: str, signed_transaction: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, str):  # Transaction signature
                return {"success": True, "signature": result, "status": "submitted"}
            else:
                return {"success": False, "error": "Transaction submission failed"}

        except Exception as e:
            logging.error(f"Error executing swap: {e!s}")
...
def create_sell_transaction(
    wallet_address: str, token_mint: str, token_amount: float, slippage: float = 0.5
) -> dict | None:
...
def generate_swap_link(
    ...
    params = [f"inputMint={input_mint}", f"outputMint={output_mint}"]
...
def create_jupiter_swap_transaction(
    private_key: str, input_mint: str, output_mint: str, amount: int, slippage_bps: int = 500
) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            return {"success": False, "error": "Solana transaction libraries not available"}
        ...
        if private_key.startswith("gAAAAAB"):
            try:
                ...
                return {"success": False, "error": f"Failed to decrypt private key: {e!s}"}
        ...
        try:
            ...
            return {"success": False, "error": f"Invalid private key format: {e!s}"}
        ...
        if not tx_data or "transaction" not in tx_data:
            return {"success": False, "error": "Failed to create Jupiter swap transaction"}
        ...
        try:
            ...
            return {"success": False, "error": f"Transaction signing/submission failed: {e!s}"}
