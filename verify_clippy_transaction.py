...
def verify_clippy_transaction():
    ...
    tx_hash = "2HzjGQbYE3uPtFMXwkbjrcUvhzkGibptqRMfRcD6oMnyFA2qaahMXfXrvDk66m9VMdf1shVm1mTbUxbBfrAswTNg"
    ...
    try:
        ...
        tx_response = client.get_transaction(signature, encoding="json", max_supported_transaction_version=0)
        ...
        if tx_response.value:
            ...
            if tx_data.meta and tx_data.meta.err is None:
                ...
                print(f"❌ Transaction failed: {tx_data.meta.err if tx_data.meta else 'Unknown error'}")
                return False
        ...
        if clippy_found and clippy_balance > 0:
            print(f"🎉 SUCCESS! TOKENS VERIFIED IN WALLET")
            ...
            print(f"❌ FAILURE: No tokens in wallet")
            print(f"❌ Transaction succeeded but delivered 0 tokens")
            print(f"❌ Same issue as before - wallet drain without tokens")
            return False
