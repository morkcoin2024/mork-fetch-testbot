...
async def admin_router(update, context):
    ...
    if not msg or not msg.text:
        logging.info("[ADMIN_ROUTER] No message or text found, skipping")
        return
    ...
    if not text.startswith("/a_"):
        logging.info("[ADMIN_ROUTER] Not an admin alias command, passing through")
    ...
    try:
        ...
        if cmd == "/a_ping":
            logging.info("[ADMIN_ROUTER] Calling cmd_ping")
            await cmd_ping(update, context)
        elif cmd == "/a_whoami":
            logging.info("[ADMIN_ROUTER] Calling cmd_whoami")
            await cmd_whoami(update, context)
        elif cmd == "/a_status":
            logging.info("[ADMIN_ROUTER] Calling cmd_status")
            await cmd_status(update, context)
        elif cmd == "/a_logs_tail":
            logging.info("[ADMIN_ROUTER] Calling cmd_logs_tail")
            await cmd_logs_tail(update, context)
        elif cmd == "/a_logs_stream":
            logging.info("[ADMIN_ROUTER] Calling cmd_logs_stream")
            await cmd_logs_stream(update, context)
        elif cmd == "/a_logs_watch":
            logging.info("[ADMIN_ROUTER] Calling cmd_logs_watch")
            await cmd_logs_watch(update, context)
        elif cmd == "/a_mode":
            logging.info("[ADMIN_ROUTER] Calling cmd_mode")
