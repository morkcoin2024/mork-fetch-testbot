...
class Rules:
    ...
    def get_filters(self, profile_name: str | None = None) -> dict[str, Any]:
        """Get filter configuration for profile"""
        profile = self.profile(profile_name)
        return profile.get("filters", {})

    def get_weights(self, profile_name: str | None = None) -> dict[str, float]:
    ...
    def calculate_score(
        ...
        return {"total": min(100.0, max(0.0, total_score)), "breakdown": score_breakdown}
    ...
    def _calculate_metric_score(
        self, token_data: dict[str, Any], metric: str, config: dict
    ) -> float:
