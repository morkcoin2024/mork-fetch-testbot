    ...
    try:
        ...
        if request.is_json:
            ...
            if 'message' in data:
                ...
                webhook_logger.info(f"[WEBHOOK] Message from user {user.get('username', 'unknown')} ({user.get('id', 'unknown')})")
    ...
    try:
        ...
        if log_path.exists():
            ...
            with log_path.open('r') as f:
                lines = f.readlines()
                recent_activity = [line for line in lines[-50:] if 'ADMIN_ROUTER' in line or 'CMD_' in line]
                
            webhook_logger.info(f"[BOT_TRACKER] Recent bot activity: {len(recent_activity)} entries")
            
            if recent_activity:
                webhook_logger.info(f"[BOT_TRACKER] Latest activity: {recent_activity[-1].strip()}")
            else:
                webhook_logger.warning(f"[BOT_TRACKER] No recent bot activity detected!")
