...
def set_rule(
    mint: str, tp: int | None = None, sl: int | None = None, trail: int | None = None
) -> dict:
    # All values optional; only update what's provided
    mint = mint.strip()
    with LOCK:
        rule = STATE["rules"].get(
            mint, {"tp": None, "sl": None, "trail": None, "ref": 0.0, "peak": 0.0}
        )
        if tp is not None:
            rule["tp"] = int(tp)
        if sl is not None:
