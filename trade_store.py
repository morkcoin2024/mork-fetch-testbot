...
def set_live(enabled: bool):
    _load(); _state["enabled_live"] = bool(enabled); _save()

def set_caps(max_sol: float = None, slippage_bps: int = None):
    _load()
    if max_sol is not None: _state["max_sol"] = float(max_sol)
    if slippage_bps is not None: _state["slippage_bps"] = int(slippage_bps)
    _save()
...
def record_fill(side: str, mint: str, symbol: str, qty: float, price: float, sol_cost: float):
    ...
    _state["fills"].append({
        "id": uuid.uuid4().hex[:8],
        "side": side, "mint": mint, "symbol": symbol,
        "qty": float(qty), "price": float(price), "sol_cost": float(sol_cost),
        "ts": time.time()
    })
    # update position
    pos = _state["positions"].get(mint, {"mint": mint, "symbol": symbol, "qty": 0.0, "avg_price": 0.0, "last_update": 0})
    if side == "BUY":
        new_qty = pos["qty"] + qty
