...
def _render_help(is_admin: bool) -> str:
    core = [
        ...
        "`/supply <MINT|TICKER>` - circulating supply (fallback: total)",
        "`/fdv <MINT|TICKER>` - fully-diluted valuation (USD)",
...
def process_telegram_command(update: dict):
    ...
    try:
        ...
        # Enhanced per-chat watchlist commands using dedicated handlers
            ...
            mint = _resolve_to_mint(target)
            if not mint:
                return _reply("Usage: `/volume <MINT|TICKER>` — unknown token.", "error")
