...
def calculate_fetch_stats(all_items, filtered_items, pumpfun_failed=False, dex_failed=False, onchain_failed=False):
    ...
    dex_early_count = len([x for x in all_items if (x.get("source") or "") in ("dexscreener-new", "dxs-new")])
...
async def send_fetch_summary(context, all_items, filtered_items, **failure_flags):
    ...
    publish("fetch.summary.generated", {
        "stats": stats,
        "sources_active": len([s for s in ["pumpfun", "dex", "onchain"] if stats.get(s, 0) > 0]),
        "efficiency_grade": "A" if stats["filter_efficiency"] > 25 else "B" if stats["filter_efficiency"] > 15 else "C"
    })